...
import { FakePostAPI } from "./api/posts";

describe("Testing Fake APi", () => {
  test("should return all posts data", async () => {
    const posts = await FakePostAPI.getAllPosts();
  ...
  test("Find post by Id", async () => {
    const res = await FakePostAPI.getPostById(1);
    expect(res).toEqual({
      userId: 1,
      id: 1,
      title:
